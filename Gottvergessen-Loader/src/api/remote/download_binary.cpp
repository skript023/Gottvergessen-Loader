...
{
	download_binary::download_binary(const std::filesystem::path& location) :
		m_location(location),
		m_latest_version(this->get_version_info()),
		m_current_version(this->get_current_version())
	{
		...
		{
			throw std::exception("Host did not return valid version data, does it have a version.json?");
		}
	...
	{
		std::ifstream fileStream(m_location, std::ios::binary | std::ios::ate);

		LOG(HACKER) << "Checking binary From Server";

		const auto file_size = fileStream.tellg();
		if (std::filesystem::exists(m_location) && file_size < 0x1000)
		{
			fileStream.close();

			LOG(WARNING) << "binary file seems inconceivably small probably file corrupted, request to inject ignored.";

			LOG(HACKER) << "Redownloading binary from server, please wait...";

			if (!this->download("Gottvergesensense", m_location))
			{
			...
			LOG(HACKER) << "New binary has been downloaded from remote, new binary version is " << m_latest_version.m_version;
		}

		if (!std::filesystem::exists(m_location))
		{
			LOG(HACKER) << "Downloading binary from server, please wait...";
			if (!this->download("Gottvergesensense", m_location))
			{
				LOG(WARNING) << "Host did not return valid version data, does it have a version.json?";
		...
		{
			LOG(HACKER) << "binary is outdated, request updating binary from server...";
			LOG(HACKER) << "Updating binary from server, please wait...";
			if (!this->download("Gottvergesensense", m_location))
			{
				LOG(WARNING) << "Host did not return valid version data, does it have a version.json?";
	...
	bool download_binary::download(const std::string filename, const std::filesystem::path& location)
	{
